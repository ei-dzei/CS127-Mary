...
<section class="panel fade-in">
  ...
  <div style="display:flex; gap:8px; flex-wrap:wrap; margin-bottom:8px;">
    <a class="btn small" href="/admin/api/export.php?table=FUNDING">Export CSV</a>
...
<section class="panel">
  ...
  <table>
    <thead><tr><th>ID</th><th>Research</th><th>Agency</th><th>Amount</th><th>Date</th><th>Actions</th></tr></thead>
    <tbody>
          ...
          <template id="tpl-edit-<?php echo (int)$row['FUNDING_ID'];?>">
            <div class="grid">
              <div class="field" style="grid-column: span 6">
                <label>Research</label>
                <select class="input" name="RESEARCH_ID" required>
                  <?php foreach($research as $r){ $sel=$r['RESEARCH_ID']===$row['RESEARCH_ID']?' selected':''; echo '<option'.$sel.' value="'.$r['RESEARCH_ID'].'">'.htmlspecialchars($r['RESEARCH_TITLE']).'</option>'; } ?>
                </select>
              </div>
              <div class="field" style="grid-column: span 4">
                <label>Agency</label>
                <select class="input" name="AGENCY_ID" required>
                  <?php foreach($agencies as $a){ $sel=$a['AGENCY_ID']===$row['AGENCY_ID']?' selected':''; echo '<option'.$sel.' value="'.$a['AGENCY_ID'].'">'.htmlspecialchars($a['AGENCY_NAME']).'</option>'; } ?>
                </select>
              </div>
              <div class="field" style="grid-column: span 2"><label>Amount (₱)</label><input class="input" type="number" step="0.01" name="FUNDING_AMOUNT" value="<?php echo htmlspecialchars($row['FUNDING_AMOUNT']); ?>"></div>
              <div class="field" style="grid-column: span 3"><label>Date Funded</label><input class="input" type="date" name="DATE_FUNDED" value="<?php echo htmlspecialchars($row['DATE_FUNDED']); ?>"></div>
            </div>
          </template>
        </td>
      </tr>
